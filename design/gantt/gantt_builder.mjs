...
const outputPath = path.join(designDir, "autofollowshoppingcart-day-gantt.xlsx");
const previewPath = path.join(designDir, "autofollowshoppingcart-day-gantt-preview.png");
const totalDays = 28;
...
const tasks = [
  ...
  },
  {
    ...
    startDay: 1,
    endDay: 5,
    deliverable: "方案 PPT 初版",
    note: "用于第 1 周阶段汇报",
  },
  {
    id: "T04",
    task: "OpenBot App 环境搭建与编译",
    owner: "成员 A",
    track: "上位机",
    startDay: 1,
    endDay: 4,
    deliverable: "可运行 OpenBot App",
    note: "记录构建环境和依赖版本",
  },
  {
    id: "T05",
    ...
    endDay: 3,
    deliverable: "可运行基础固件",
    note: "打通 Arduino Nano 或 ESP32 基础控制",
  },
  {
    id: "T06",
    task: "底盘、电机、电源到货检查",
  ...
  },
  {
    ...
    startDay: 5,
    endDay: 7,
    deliverable: "手动遥控测试记录",
    note: "形成第 1 周基础链路验收证据",
  },
  {
    id: "T11",
    task: "人物检测与目标初始化",
  ...
  },
  {
    ...
    startDay: 14,
    endDay: 18,
    deliverable: "LOST / SEARCH / STOP 联调完成",
    note: "先取消前进，再短时原地搜寻，超时停车",
  },
  {
    id: "T19",
    task: "障碍停车集成",
  ...
  },
  {
    ...
    startDay: 23,
    endDay: 27,
    deliverable: "海报、PPT、总结报告",
    note: "成员 C 主收口，A/B 提供技术内容",
  },
  {
    id: "T26",
    task: "最终彩排与现场预案确认",
...
ganttSheet.getRange("A2:F2").format.wrapText = true;
...
for (let i = 0; i < tasks.length; i += 1) {
  const row = baseRow + i;
  ganttSheet.getRange(`C${row}`).format.fill = ownerColors[tasks[i].owner] || "#F8FAFC";
...
for (let i = 0; i < tasks.length; i += 1) {
  ...
  for (let day = 0; day < totalDays; day += 1) {
    const colLetter = excelColName(7 + day);
    formulas.push(`=IF(AND(${colLetter}$4>=$E${row},${colLetter}$4<=$F${row}),1,"")`);
  }
  ganttSheet.getRange(`G${row}:${lastDayCol}${row}`).formulas = [formulas];
}
...
ganttSheet
  .getRange(`G${baseRow}:${lastDayCol}${baseRow + tasks.length - 1}`)
...
ganttSheet.getRange(`A${noteRow}`).values = [[
  "说明：本甘特图使用第 1 天至第 28 天的相对排期，不绑定自然日。蓝色条表示计划主工作日，浅灰底用于标识每周后两天的节奏参考。",
]];
...
ganttSheet.getRange(`A${noteRow + 1}:C${noteRow + 3}`).values = [
  ["并行主线", "成员 A", "OpenBot App、人物检测、目标锁定、跟随参数调节"],
  ["并行主线", "成员 B", "MCU、通信、电机驱动、供电与急停执行"],
  ["并行主线", "成员 C", "结构改造、安全测试、视频记录与材料收口"],
];
ganttSheet.getRange(`A${noteRow + 1}:C${noteRow + 3}`).format = {
  borders: { preset: "all", style: "thin", color: "#CBD5E1" },
  wrapText: true,
};
...
ganttSheet.getRange(`A${noteRow}:C${noteRow + 3}`).format.autofitRows();
